        ...
        const fetchProfile = async () => {
            try {
                ...
                if (response.status === 403) {
                    setError({ message: "Access Denied: You do not have permission to view this user's profile.", status: 403 });
                    setLoading(false);
                    return;
                }
        ...
        return (
            <div className="min-h-screen bg-[#09090b] text-[#f4f4f5] flex flex-col justify-center items-center p-6 relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-md w-full bg-zinc-900/60 backdrop-blur-xl border border-red-500/20 rounded-2xl p-8 text-center shadow-2xl">
                    ...
                    <p className="text-zinc-400 text-sm mb-6 leading-relaxed">{error.message || "Failed to load profile."}</p>
                    <div className="flex gap-4 justify-center">
                        {isAuthError ? (
                            ...
                            <>
                                ...
                                <button onClick={handleLogout} className="px-5 py-2.5 bg-zinc-950 border border-zinc-800 hover:bg-zinc-900 text-zinc-400 rounded-xl transition font-medium text-sm">
                                    Log Out
                                </button>
                            </>
                        ) : (
